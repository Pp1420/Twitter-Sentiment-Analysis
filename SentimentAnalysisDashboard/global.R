...
library(tidytext); library(tidyverse); library(DT);
library(fontawesome); library(dplyr); library(tm);
library(knitr); library(GGally); library(stringr);
library(tidyr); library(sentimentr); library(quanteda);
library(wordcloud); library(wordcloud2); library(caTools);
library(syuzhet); library(lubridate); library(ggplot2);
library(scales); library(reshape2); library(rpart);
library(rpart.plot); library(ggrepel); library(text2vec);
library(randomForest); library(stats); library(e1071);
library(caret); library(lattice); library(psych);
library(shiny); library(shinydashboard); library(xgboost);
library(webshot); library(htmlwidgets); library(gridExtra);
library(readr); library(curl); library(shinyauthr); library(viridis)
library(tinytex)
...
Tweets = subset(Tweets, select = c(Tweet))
str(Tweets)
dim(Tweets)
...
ggplot(df, aes(x = reorder(words, desc(freq)), y = freq, fill = words)) +
  geom_col(width = 0.5, color = "black") +
  labs(title = "Top 20 Words", y = "Frequency") +
  xlab("Words") + # Add this line to label the x-axis
  theme_minimal() +
...
# E.g. score of -0.286, indicating that the overall sentiment of the tweet 
 #is negative
brkBingTweet %>% print(n=5)

positive_total <- sum(brkBingTweet$positive)
negative_total <- sum(brkBingTweet$negative)

if (positive_total > negative_total) {
  cat("Positive outweighs negative.\n")
} else {
  cat("Negative outweighs positive.\n")
}
...
ggplot(brkBingTweet, aes(TweetNum, Score, fill = Score > 0)) +
  geom_col(show.legend = FALSE) +
  scale_fill_manual(values = c("lightcoral", "blue")) +
...
# Get top 20 positive sentiment words
pos_words <- brkBing %>%
  filter(sentiment == "positive") %>%
  count(word, sentiment, sort = TRUE) %>%
  top_n(20)
...
#Sentiment Polarity according to AFINN Lexicon
brkAFINN <- brkwords %>%      
